  ...
  for seg_id in segment_ids:
    ...
    for section_id in section_ids:
      ...
      processed.append({
        ...
        # Here we keep the full flight code as provided.
...
def simplify_edreams_json(input_filepath, output_filepath):
  ...
  for itinerary in itineraries:
    ...
    sort_price = itinerary.get("price", {}).get("sortPrice", 0.0)

    # Extract membership perks fee and compute primePrice.
    membershipPerks = itinerary.get("membershipPerks", {})
    membership_fee = membershipPerks.get("fee", 0.0)
    prime_price = float(sort_price) + float(membership_fee)
