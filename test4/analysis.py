...
count_cheaper_non_repeated_kiwi = sum(1 for entry in kiwi_data if entry["id"] in not_repeated_kiwi and float(entry["price"]) < cheapest_edreams_price)
kiwi_cheaper_count = count_cheaper_non_repeated_kiwi + kiwi_cheaper_count
total_itineraries = len(kiwi_data) + len(edreams_data) - len(repeated_itineraries)
percent_kiwi_cheaper_overall = 100 * kiwi_cheaper_count / total_itineraries
